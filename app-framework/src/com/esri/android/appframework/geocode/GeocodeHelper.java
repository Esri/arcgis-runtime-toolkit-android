...
/**
 * Helper class providing static methods to generate candidates for an address or 
 * find an address for a given location. These methods simplify the workflow of
 * setting parameters and getting result from a Locator. The operation can be cancelled 
 * through Future. When an operation is cancelled, the the callback won't be invoked. However,
 * the request to the operation may be sent before the operation is cancelled.
 * 
 * @since 10.2
 */
...
public class GeocodeHelper {
  ...
  * @param mapView the MapView. When mapView is null, null will be returned.
  ...
  */
  public static Future<LocatorReverseGeocodeResult> findAddress(float screenX, float screenY, Locator locator, MapView mapView, CallbackListener<LocatorReverseGeocodeResult> callback) {
    ...
    return Locator.findAddress(point2.getY(), point2.getX(), locator, callback);
  }
   ...
   * 
   * @param name the address to lookup.
   * @param locator the geocode service of ArcGIS Server to use. If null, 
   *          a default locator using ESRI ArcGIS online worldwide geocoding service will be used.
   *          The endpoint of this service is "http://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer".   
    ...
    return TaskExecutor.pool.submit(new Callable<List<LocatorGeocodeResult>>() {
      ...
      public List<LocatorGeocodeResult> call() {
        if (numToShow < 1)
          return null;
        try {
          final Locator loc = locator != null ? locator : Locator.createOnlineLocator();
          LocatorFindParameters findParams = new LocatorFindParameters(name);
          findParams.setMaxLocations(numToShow);
          final List<LocatorGeocodeResult> results = loc.find(findParams);
  ...
  /**
   * Finds an address for a given location using screen coordinates and add to the MapView as a marker graphic.
   ...
   *          An empty callout window will be display if fieldsToShow is null.
   * @param callback the callback to invoke when the operation is complete or on error.
   * @return a Future which resolves to a LocatorReverseGeocodeResult if the operation is complete.
   */
  ...
  /**
   * Finds an address for a given location using lat/lon and add to the MapView as a marker graphic.
   * When tapping on the marker graphic the address will be displayed in a callout window.
   * The marker graphic and the associated callout window can be manipulated through MapView.
   * 
  ...
  public static Future<LocatorReverseGeocodeResult> showAddress(final double lat, final double lon, final Locator locator, final MapViewHelper mapViewHelper, final Drawable icon, final String[] fieldsToShow, final CallbackListener<LocatorReverseGeocodeResult> callback) {
    ...
    return TaskExecutor.pool.submit(new Callable<LocatorReverseGeocodeResult>() {
      ...
      public LocatorReverseGeocodeResult call() {
        try {
          final Locator loc = locator != null ? locator : Locator.createOnlineLocator();
          SpatialReference sr = SpatialReference.create(SpatialReference.WKID_WGS84);
          final LocatorReverseGeocodeResult result = loc.reverseGeocode(new Point(lon, lat), TOLERANCE, sr, sr);
          if (Thread.currentThread().isInterrupted())
            return result;
          if (callback != null)
          ...
          if (result != null) {
            mapViewHelper.getMapView().post(new Runnable() {
              ...
              public void run() {
                ...
                if (addressFields != null && !addressFields.isEmpty() && fieldsToShow != null && fieldsToShow.length > 0) {
                  for (String field : fieldsToShow) {
                    if (addressFields.containsKey(field))
